...
const productCategories = [
  {
    title: 'Network / Data Centre Solutions',
    items: ['Ethernet Switch', 'Industrial Grade Switch', 'SFP Module', 'MPO / MTP Cable', 'Media Converter']
  },
  {
    title: 'Fiber Optic Solutions',
    items: ['OFC Cable', 'FDMS / LIU', 'FODP', 'Joint Enclosure', 'Patch Cord', 'Splitter', 'Splice Protection Sleeve', 'Optical Attenuator']
  },
  {
    title: 'Structured Cabling',
    items: ['CAT 6/6A Cable', 'UTP/STP Patch Panel', 'CAT 6/6A Patch Cord', 'Face Plate', 'Keystone Jack', 'I/O Box', 'Network / Server Rack']
  }
]
  ...
  return (
    <header className="sticky top-0 z-[999] flex items-center justify-between px-16 py-4 border-b border-cyan/10 bg-blue-deep/95 backdrop-blur-md">
      ...
      <nav className="hidden lg:flex items-center gap-1" style={{position:'relative',zIndex:999}}>
        {navLinks.map(link => (
          <Link key={link.href} href={link.href}
            className={`text-xs tracking-widest uppercase px-3 py-2 transition-colors hover:text-cyan ${pathname === link.href ? 'text-cyan' : 'text-text-muted'}`}>
            {link.label}
          </Link>
        ))}

        <div style={{position:'relative',zIndex:999}} onMouseEnter={() => setProductsOpen(true)} onMouseLeave={() => setProductsOpen(false)}>
          <button className={`flex items-center gap-1 text-xs tracking-widest uppercase px-3 py-2 transition-colors ${pathname.startsWith('/products') ? 'text-cyan' : 'text-text-muted hover:text-cyan'}`}>
            Products
            <svg className={`w-3 h-3 transition-transform ${productsOpen ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
              <polyline points="6 9 12 15 18 9"/>
            </svg>
          </button>
          <div style={{position:'absolute',top:'calc(100% + 8px)',left:'50%',transform:productsOpen?'translateX(-50%) translateY(0)':'translateX(-50%) translateY(8px)',width:'720px',background:'rgba(4,15,32,0.98)',border:'1px solid rgba(0,229,255,0.15)',borderRadius:'8px',padding:'24px',opacity:productsOpen?1:0,visibility:productsOpen?'visible':'hidden',transition:'all 0.25s ease',backdropFilter:'blur(20px)',boxShadow:'0 30px 80px rgba(0,0,0,0.8)',zIndex:9999}}>
            ...
            <div className="grid grid-cols-3 gap-0">
              {productCategories.map((cat, i) => (
                <div key={cat.title} style={{padding:'0 18px',paddingLeft:i===0?0:undefined,borderRight:i<2?'1px solid rgba(0,229,255,0.08)':'none'}}>
                  <div className="text-[10px] tracking-[0.26em] uppercase text-cyan font-orbitron font-semibold mb-3 pb-2 border-b border-cyan/10">{cat.title}</div>
                  {cat.items.map(item => (
            ...
            <div className="mt-4 pt-4 border-t border-cyan/08 text-right">
              <Link href="/products" className="font-orbitron text-[10px] tracking-widest uppercase text-cyan hover:text-white transition-colors">
                View All Products →
              </Link>
            </div>
          </div>
      ...
      <div className="hidden lg:flex items-center gap-3">
        <Link href="/contact" className="font-orbitron text-[11px] tracking-widest uppercase bg-cyan text-blue-deep px-5 py-2 rounded-sm font-bold hover:bg-cyan-dim transition-colors">
          Get Quote
        </Link>
      ...
      {mobileOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-blue-dark/98 border-b border-cyan/10 p-6" style={{zIndex:999}}>
          <div className="flex flex-col gap-4">
            ...
            <Link href="/partner" className="text-sm text-text-muted hover:text-cyan transition-colors" onClick={() => setMobileOpen(false)}>Partners</Link>
            ...
            <Link href="/contact" className="font-orbitron text-[11px] tracking-widest uppercase bg-cyan text-blue-deep px-5 py-2 rounded-sm font-bold text-center" onClick={() => setMobileOpen(false)}>Get Quote</Link>
          </div>
        </div>
      )}
    </header>
