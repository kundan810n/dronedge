...
const defaultTestimonials = [
  ...
  { name: 'Vijay Kumar', role: 'Project Manager, Smart City Project', text: 'We\'ve been using Dron Edge switches for our smart city project. The build quality and after-sales support is top notch. Highly recommended!', initials: 'VK' },
      ...
      <div className="flex gap-6 w-max" style={{animation:'scrollBanner 45s linear infinite'}}
        ...
        {doubled.map((t, i) => (
          <div key={i} className="flex-shrink-0 w-[360px] rounded-lg p-5"
            ...
            <p className="text-xs text-text-muted font-light leading-relaxed italic mb-4 line-clamp-3">{t.text}</p>
            <div className="flex items-center gap-3 pt-3" style={{borderTop:'1px solid rgba(0,229,255,0.08)'}}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center font-orbitron text-[10px] font-bold text-cyan flex-shrink-0"
                style={{background:'linear-gradient(135deg,rgba(0,80,180,0.5),rgba(0,229,255,0.2))',border:'1px solid rgba(0,229,255,0.2)'}}>
                {t.initials}
              </div>
              <div>
                <div className="font-orbitron text-[11px] font-semibold text-white">{t.name}</div>
